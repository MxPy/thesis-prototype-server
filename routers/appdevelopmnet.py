...
async def show_student(id: str, db: Session = Depends(get_no_sql_db)):
    """
...
async def show_student(id: str, db: Session = Depends(get_no_sql_db)):
    ...
    """
    if (
        student := await db.find_one({"session_id": id})
    ) is not None:
        return student

    raise HTTPException(status_code=404, detail=f"Student {id} not found")

@router.delete("/session/{id}", response_description="Delete a student")
async def delete_student(id: str, db: Session = Depends(get_no_sql_db)):
    """
...
async def create_student(student: models.Session = Body(...), db: Session = Depends(get_no_sql_db)):
    """
